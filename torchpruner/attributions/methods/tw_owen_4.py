...
class GroupedOwenShapleyAttributionMetric(_AttributionMetric):    
    ...
    def run(self, module, **kwargs):    
        ...
        # 检查是否支持forward_partial并给出相应提示  
        if hasattr(self.model, "forward_partial"):  
            return self.run_module_grouped_owen_with_partial(module)  
        else:  
    ...
    def run_module_grouped_owen_with_partial(self, module):  
        """  
        Implementation of grouped Owen sampling using forward_partial for better performance.  
        """  
        sv_results = []  
        ...
        with torch.no_grad():  
            for batch_idx, (x, y) in enumerate(self.data_gen):  
                ...
                for sample_idx in range(batch_size):  
                    sample_sv = self._compute_grouped_owen_shapley_single_with_partial(  
    ...
    def _compute_grouped_owen_shapley_single_with_partial(self, x_single, y_single, module, n_features):  
        """  
        使用forward_partial优化的单样本分组Owen Shapley值计算  
        """  
                ...
                for _ in range(self.runs):    
                    for q1_num in range(self.q_splits):    
                        ...
                        for q2_num in range(self.q2_splits):    
                            ...
                            loss_s = self._evaluate_coalition_with_partial(x_single, y_single, module, coalition_s)    
                            loss_s_union_j = self._evaluate_coalition_with_partial(x_single, y_single, module, coalition_s_union_j)    
    ...
    def _compute_grouped_owen_shapley_single(self, x_single, y_single, module, n_features):  
        """  
        原始的单样本分组Owen Shapley值计算（使用hook方式）  
        """  
        phi = np.zeros(n_features)    
        groups = []    
        for i in range(0, n_features, self.group_size):    
    ...
    def _evaluate_coalition_with_partial(self, x_single, y_single, module, coalition_mask):  
        """  
        使用forward_partial优化的联盟评估方法  
        """  
        # 获取到目标模块的激活  
        z, _ = self.run_forward_partial(x_single, to_module=module)  
        ...
        masked_z = z * mask_tensor  
          
        # 从目标模块继续前向传播  
        _, loss = self.run_forward_partial(masked_z, y_true=y_single, from_module=module)  
        return loss.item()  
